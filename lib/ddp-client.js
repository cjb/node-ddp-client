...
DDPClient.prototype._message = function(data, flags) {
  ...
  // missing subscription
    ...
    if (data.collection) {
      ...
      var update_cb = self._update_callbacks[data.collection];
      if (update_cb)
        update_cb(data);
...
DDPClient.prototype.subscribe = function(name, params, callback, update_cb, collection) {
  var self = this;
  var id = self._nextId()
  ...
  if (callback)
    self._callbacks[id] = callback;
  if (update_cb)
    self._update_callbacks[collection] = update_cb;
